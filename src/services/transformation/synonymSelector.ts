...
export class SynonymSelector {
  ...
  async selectBestSynonym(
    synonyms: RankedSynonym[],
    ...
    const varietyAdjustedSynonyms = this.applyVarietyLevel(
      synonyms,
      options.varietyLevel,
      options.creativity
    );
  ...
  private applyVarietyLevel(
    ...
    const count = Math.max(
      3,
      Math.ceil(synonyms.length * (varietyLevel * creativity))
    );

    return varietyAdjusted.slice(0, count);
  }

  private adjustScoreForVariety(
    synonym: RankedSynonym,
    varietyLevel: number
  ): number {
  ...
  private async calculateContextScore(
    ...
    const scores: SynonymScores = {
      emotionalToneScore: await this.emotionalAnalyzer.analyzeSynonym(
        synonym,
      ...
      contextScore:
        this.getContextPreservationScore(synonym, context) *
        options.contextPreservation,
    };

    return (
      scores.emotionalToneScore * 0.3 +
      scores.formalityScore * 0.3 +
      scores.contextScore * 0.4
    );
  }

  private getFormalityScore(
    synonym: SynonymResult,
    formality: 'formal' | 'informal'
  ): number {
    const formalPatterns =
      /^(implement|utilize|facilitate|optimize|establish)/i;
