...
export class EmotionalToneAnalyzer {
  ...
  async analyzeSynonym(
    synonym: SynonymResult,
    targetTone: EmotionalTone,
    contextWord: string
  ): Promise<number> {
    const baseScore = this.calculateEmotionalScore(
      synonym,
      targetTone,
      contextWord
    );
    ...
    const finalScore =
      baseScore * 0.5 + intensityScore * 0.3 + contextScore * 0.2;

    return Math.max(
      CONFIDENCE_THRESHOLDS.MIN_EMOTIONAL_SCORE,
      Math.min(1, finalScore)
    );
  }
