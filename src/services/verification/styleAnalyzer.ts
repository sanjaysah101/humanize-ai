...
export class StyleAnalyzer {
  ...
  async evaluate(text: string): Promise<number> {
    ...
    return (
      Object.values(styleMetrics).reduce((acc, score) => acc + score, 0) / 3
    );
  }
  ...
  private analyzeFormalityLevel(text: string): number {
    ...
    return Math.max(
      CONFIDENCE_THRESHOLDS.MIN_WORD_SCORE,
      1 - informalCount / words
    );
  }
  ...
  private analyzeStyleConsistency(text: string): number {
    ...
    const voicePatterns = sentences.map((s: string) =>
      this.nlpProcessor(s).has('#Passive')
    );
    const voiceChanges = voicePatterns.filter(
      (v: boolean, i: number) => i > 0 && v !== voicePatterns[i - 1]
    ).length;
